...
class DynGraphTrainer(BaseTrainer):
    ...
    def build_model(self):
        if self.config.model == "gcn":
            self.model = GCN_Graph_Sequence(len(self.feature_list), self.config.hidden, 2, 0.75, "sagpool", "mean", "lstm").to(self.config.device)
        elif self.config.model == "gin":
            self.model = GIN_Graph_Sequence(None, len(self.feature_list), 2, "sagpool", "mean", "lstm", self.config.num_layers).to(self.config.device)
