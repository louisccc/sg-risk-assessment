...
class HUD(object):
    def __init__(self, width, height):
        ...
        default_font = 'arial'
        mono = default_font if default_font in fonts else fonts[2]
    ...
    def tick(self, world, clock):
        ...
        if len(vehicles) > 1:
            ...
            vehicles = [(velocity(x.get_velocity()), distance(x.get_location()), x) for x in vehicles if x.id != world.player.id]
            #vehicles = [(distance(x.get_location()), x) for x in vehicles if x.id != world.player.id]
            for v, d, vehicle in sorted(vehicles):
                if d > 100.0:
                    continue
                else:
                    vehicle_type = get_actor_display_name(vehicle, truncate=22)
                    #self._info_text.append('% 4dm %s' % (d, vehicle_type))
                    self._info_text.append('% 3d km/h, % 4dm %s' % (int(v), d, vehicle_type))
